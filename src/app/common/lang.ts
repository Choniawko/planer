export const lang = {
        defaultLang: 'pl',
        lang: {
            // general
        'TRUE': { en: 'True', pl: 'prawda' },
        'FALSE': { en: 'False', pl: 'Fałsz' }, 
      
            // user
        'NAME': { en: 'Username', pl: 'Nazwa użytkownika' },
        'PASSWORD': { en: 'Password', pl: 'Hasło' }, 
        'REPEAT_PASSWORD': { en: 'Repeat password', pl: 'Powtórz hasło' },
        'EMAIL': { en: 'Email', pl: 'Email' },
        'FIRSTNAME': { en: 'First name', pl: 'Imię' },
        'LASTNAME': { en: 'Last name', pl: 'Nazwisko' },
        'REQUIRED': { en: 'Field is required', pl: 'Pole wymagane' },
        'WRONG_FORMAT': {en: 'Wrong format', pl: 'Zły format pola'},
        'PASS_NOT_EQUAL': {en: 'Passwords not equal', pl: 'Hasła nie są identyczne'},
        'NEW_USER': {en: 'New user', pl: 'Nowy użytkownik'},
        'USER_LIST': {en: 'User list', pl: 'Lista użytkowników'},
        'LOGIN': {en: 'Login', pl: 'Zaloguj się'}
       
        }
    };